...
import {GetPaymentInfoAction, PaymentAction} from 'src/store/Payment/action';
...
class PaymentScreen extends React.Component {
  ...
  onPay = async() => {
    ...
    this.props.payment (data, this.props.token)
  }
  componentDidMount () {
    if (!this.props.token) {
    ...
    this.focusListener = navigation.addListener("didFocus", () => {
        this.setState ({hashedId: navigation.state.params.hashed_id })
        console.log ("paymentscreen...")
        this.props.getPaymentInfo (navigation.state.params.hashed_id , this.props.token)
    })
  }
    ...
    return (
        <ScrollView>
          ...
          <Container>    
            <Menu title="Payment"/>
            <Footer />
            <Title>  payment card for this estimate</Title>
            <Footer />
...
const mapDispatchToProps = (dispatch) => {
  return {
    getPaymentInfo: (hashedId, token) => dispatch(GetPaymentInfoAction(hashedId, token)),
    payment: (data, token) => dispatch(PaymentAction(data, token))
  };
};
