...
class PaidInvoiceScreen extends Component {
  ...
  componentDidMount () {
    ...
    this.focusListener = navigation.addListener("didFocus", () => {
      this.props.getInvoice (1, this.props.token)
    });
  }
          ...
          <Menu title="Paid" />
            ...
            <MyTable>
              <DataTable.Row>
                ...
                <HeaderRight>Status</HeaderRight>
              </DataTable.Row>
              {
                this.props.data && this.props.data.invoices && this.props.data.invoices.map ((item, index) => {
                  return (
                    <TableRow key={index}>
                      ...
                      <RightCell>
                      <Text>{item.is_paid == 1? 'paid': 'not paid' }</Text>
                      </RightCell>
                    </TableRow>
                  )
                })
...
export default connect(mapStateToProps, mapDispatchToProps)(PaidInvoiceScreen);
